...
public class enemyController : MonoBehaviour {
	...
	//for collision with 'slice' (players sword) pushes enenmy away from player
	void OnTriggerEnter2D (Collider2D col)
	{
		if(col.gameObject.name == "Slice(Clone)")
		{
			//setting up force to add to rigid body
			Vector3 dir = (transform.position - myScript.charPosition).normalized;
			dir *= speed * Time.fixedDeltaTime * 120f;
			//adds force to this rigidbody
			rb.AddForce(dir, fMode);
		}
	}

	//for collision with walls, dies or gets destoryed when wall is it
